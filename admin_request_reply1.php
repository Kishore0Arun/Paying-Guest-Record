...
<?php
	...
	require("includes/connect.php");
		...
		{					
			...
			$q1 = $con->prepare("SELECT * FROM `customer` WHERE cid = ?");
			$q1->bind_param('s', $cid);
			$q1->execute();
			$res1 = $q1->get_result();
			...
			$q2 = $con->prepare("UPDATE `room` SET availability = ? WHERE rid = ?");
			$q2->bind_param('ss', $status, $rid);
			$q2->execute();
			
			?>
				<div role="alert" class="alert alert-success"> 
					<strong>Well done!!</strong> Customer request accepted...
				</div>
